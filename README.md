...
`com.gios.lightsports`. Current released version: **v2.13**.
...
| Version | Change |
| --- | --- |
| v2.16 | **The celebration buzzes, in its own shape.** `anim/Buzz.kt` gives each style a waveform cut to its own beats — three cracks for Mortar (on the shells' burst frames, then silence while the sparks fall), a stepped swell and a snap for Grid, two events for Score burst, one fading tap per ring for Halftone — folded into a strictly alternating silence/motor array so `createWaveform(timings, -1)` is still correct on a phone with no amplitude control. `notify/Buzzer.kt` is now the only thing that touches the motor: one clock, one rate limit, and a priority where a celebration takes it off the alert's double tick but never the reverse, since the two could otherwise fire within a second and overlap into neither. Tests pin the total motor time under 400 ms and check every beat lands on the frame it belongs to. Silent ringer mode skips it; vibrate mode does not. `Buzz with it` in settings, on by default |
| v2.15 | **The screen celebrates a score.** Four styles in `model/Celebration.kt` — Mortar (three staggered shells), Grid (the SDK's own 27 × 31 lit as a wave and dropped cell by cell), Score burst (the old figure sampled out of the condensed face and thrown), Halftone (rings out from the number that moved). The arithmetic is `anim/Fireworks.kt`, with no Android in it and a test per effect; `ui/CelebrationOverlay.kt` draws it on a `drawBehind` layer reading a `MutableLongState`, so a run costs draw frames and no recompositions. Brightness is an ordered 4×4 dither rather than an alpha — the matte glass turns soft edges to mush, and on an AMOLED a lit pixel is the cost, which is also why `Grid.PEAK` caps the field at 0.55 and a test pins it. Fires on a followed, unsilenced side's score rising and once on a FINAL win; off by default, and each settings row plays its own style on tap |
